...
import { setupTotp, confirmTotpSetup, disableTotp } from '../services/auth.service';
import { useAuth } from '../contexts/AuthContext';
import type { TotpSetupData } from '../types/auth';
...
export function TotpSetupDialog() {
  ...
  const handleSetup = async () => {
    ...
    setError('');
    try {
      const data = await setupTotp('session');
      setSetupData(data);
      setStep('qr');
    } catch (err) {
  ...
  const handleConfirm = async () => {
    ...
    setError('');
    try {
      await confirmTotpSetup('session', code);
      setStep('done');
    } catch (err) {
      setError(getApiErrorMessage(err));
      setCode('');
  ...
  const handleDisable = async () => {
    ...
    setError('');
    try {
      await disableTotp('session', code);
      setStep('done');
    } catch (err) {
      setError(getApiErrorMessage(err));
      setCode('');
